    ...
    def evaluate_rules(rules_map, entity_attributes, feature, property, entity_id = nil)
      ...
      begin
        ...
        (1..rules_map.keys.length).each do |index|
          ...
          next unless segment_rule.rules.length.positive?
            ...
            segments.each do |segment_key|
              ...
              if feature
                ...
                if segment_level_rollout_percentage == 100 ||
                  ...
                  result_dict[:value] = if segment_rule.value == Constants::DEFAULT_FEATURE_VALUE
                                          feature.enabled_value # Return the inherited value
                                        else
                                          segment_rule.value # Return the overridden value
                                        end
                ...
                result_dict[:value] = if segment_rule.value == Constants::DEFAULT_PROPERTY_VALUE
                                        property.value
                                      else
                                        segment_rule.value
                                      end
